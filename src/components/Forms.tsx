...
const Forms = ({ language = 'en' }) => {
  ...
  const getRequiredFields = useCallback(async () => {
    try {
      ...
      const { body } = restOperation.response;
      // const responseText = await body.text();
      // const response = JSON.parse(responseText);
    ...
    return (
      <div className="min-h-screen p-4 text-white">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="text-2xl font-bold">{t('analyzingDocuments')}</h1>
          <button
            onClick={() => {
          ...
          <div className="space-y-4">
            {analyzedFiles.map((file, index) => (
              <motion.div
                ...
                className="bg-purple-900/20 p-4 rounded-lg flex items-center justify-between"
              >
    ...
    return (
      <div className="min-h-screen p-4 text-white">
        <div className="mb-2 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <h1 className="text-2xl font-bold">{t('formAssistant')}</h1>
            <button
        ...
        <div className="grid grid-cols-2 gap-4 h-[calc(100vh-8rem)] mb-4">
          <ScrollArea className="bg-purple-900/20 rounded-lg flex flex-col">
            <div className="p-4 flex-1 flex flex-col">
              <ScrollArea className="flex-1 -mr-4 pr-4 space-y-4">
                {formFields.fields && Object.entries(formFields.fields).map(([fieldName, description]) => (
                  <div key={fieldName} className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-purple-900/30 rounded-lg px-4 py-2 
          ...
          <div className="bg-purple-900/20 rounded-lg pb-4">
            <embed
              ...
              src="https://extra-documents-government.s3.us-east-1.amazonaws.com/ar-11-filled.pdf?v=${Date.now()}"
          ...
          <AnimatePresence>
            {showDropdown && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="absolute right-0 mt-2 w-48 bg-purple-900/90 rounded-lg 
                         shadow-lg overflow-hidden z-50"
              >
                <button
                  onClick={() => handleFormSelect('AR-11')}
                  className="w-full text-left px-4 py-2 text-white hover:bg-purple-700 
                           transition-colors"
                >
                  AR-11
                </button>
